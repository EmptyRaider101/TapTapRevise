...
interface AppState {
  ...
  setAIModel: (
    model: "gemini-3.1-flash-lite-preview" | "gemma-4-31b-it"
  ) => void
...
export const useStore = create<AppState>((set, get) => ({
  ...
  init: async () => {
    try {
      const [filesRes, notesRes, windowsRes, settingsRes, quizzesRes, sessionsRes, resultsRes] = await Promise.all([
        axios.get(`${API_BASE}/files`),
        axios.get(`${API_BASE}/notes`),
        axios.get(`${API_BASE}/windows`),
        axios.get(`${API_BASE}/settings`),
        axios.get(`${API_BASE}/quizzes`).catch(() => ({ data: [] })),
        axios.get(`${API_BASE}/quiz-sessions`).catch(() => ({ data: [] })),
        axios.get(`${API_BASE}/quiz-results`).catch(() => ({ data: [] })),
      ])
      ...
      set({
        ...
        windows: (serverWindows.length > 0 ? serverWindows : DEFAULT_WINDOWS).filter(
          (w: any) => w.type !== "quiz-creator"
        ),
        ...
        examDate: serverSettings.examDate
          ? parseInt(serverSettings.examDate)
          : null,
        dailyGoal: serverSettings.dailyGoal
          ? parseInt(serverSettings.dailyGoal)
          : 5,
  ...
  updateFileStatus: async (id, status) => {
    ...
    try {
      ...
      if (wasUnread && isNowRevised) {
        ...
        const revisedToday = updatedFiles.filter(
          (f) =>
            f.revisedAt &&
            new Date(Number(f.revisedAt)).setHours(0, 0, 0, 0) === today
        ).length
    ...
    try {
      ...
      set((state) => {
        ...
        return {
          notes: [
            ...state.notes,
            { id: uuidv4(), fileId, content, updatedAt: Date.now() },
          ],
        }
      })
    } catch (e) {
  ...
  saveQuiz: async (quiz) => {
    try {
      await axios.post(`${API_BASE}/quizzes`, quiz)
  ...
  saveQuizSession: async (session) => {
    try {
      await axios.post(`${API_BASE}/quiz-sessions`, session)
      set((state) => {
        const existingIdx = state.quizSessions.findIndex((s) => s.quizId === session.quizId)
        if (existingIdx > -1) {
          const next = [...state.quizSessions]
          next[existingIdx] = session
          return { quizSessions: next }
        }
        return { quizSessions: [...state.quizSessions, session] }
      })
    } catch (e) {
      console.error("Save quiz session failed:", e)
    }
  },

  deleteQuizSession: async (quizId) => {
    try {
      await axios.delete(`${API_BASE}/quiz-sessions/${quizId}`)
      set((state) => ({
        quizSessions: state.quizSessions.filter((s) => s.quizId !== quizId)
      }))
    } catch (e) {
      console.error("Delete quiz session failed:", e)
    }
  },

  saveQuizResult: async (result) => {
    try {
      await axios.post(`${API_BASE}/quiz-results`, result)
      set((state) => ({
